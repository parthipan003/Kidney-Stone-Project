...
def predict_image(image):
    ...
    for i, (x, y, w, h) in enumerate(xywh):
        ...
        cv2.putText(img, label, (int(x1_expanded), int(y1_expanded)-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

        color_hex = color_to_hex(color)
        color_swatch = f'<div style="width:20px; height:20px; background-color:{color_hex};"></div>'
        ...
        predictions.append({
            ...
            "Color": color_swatch  # Add color swatch as HTML
        })
    ...
    # Convert DataFrame with HTML for color swatches
    df = pd.DataFrame(predictions).to_html(escape=False)
...
iface = gr.Interface(
    ...
    outputs=[
        ...
        gr.HTML(),  # Render HTML DataFrame with color swatches
